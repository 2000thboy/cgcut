...
if not errorlevel 1 (
    echo [警告] 端口8000已被占用，请先停止相关进程
)
netstat -an | findstr :8001 >nul
if not errorlevel 1 (
    echo [警告] 端口8001已被占用，请先停止相关进程
)
netstat -an | findstr :5173 >nul
if not errorlevel 1 (
    echo [警告] 端口5173已被占用，请先停止相关进程
)
...
echo ============================================
echo 在新窗口中启动 CLIP 服务...
start "CLIP Service" cmd /k "cd /d %~dp0clip-service && python clip_server.py"
...
echo ============================================
echo 在新窗口中启动 VLM 服务...
start "VLM Service" cmd /k "cd /d %~dp0vlm-service && python vlm_server.py"
